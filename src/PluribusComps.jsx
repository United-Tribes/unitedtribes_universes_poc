...
function buildCrewDetailPrompt(name, entity, universe) {
  ...
  const interviews = (entity?.interviews || []).map(iv => `"${iv.title}" (${iv.meta || iv.type}${iv.context ? ` — ${iv.context}` : ""})`).join("; ");
  const videos = ((entity?.quickViewGroups || []).find(g => g.label === "Videos")?.items || []).map(v => `"${v.title}" (${v.meta})`).join("; ");
...
QUOTE: "[Quote text]" — ${name}, [full source title including episode name/number if applicable] [VIDEO:video_id_here] [TIME:timestamp_in_seconds]

The video_id and timestamp should come from the interview data provided above. Match each quote to its source interview/video entry. If you don't know the exact video_id or timestamp, omit those tags but still include the source title.
